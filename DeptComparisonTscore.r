# Req: Here is how your Department X compares with all other Department X's across the Pivot dataset converting to Tscores
...
#Calculate overall mean of student responses over dataset
OverallMean <- mean(RespData$answer_num)

#Calculate overall standard deviation of student responses over dataset
OverallSD <- sd(RespData$answer_num)

# Create a new columns to store the Z score and T score
RespData$Zscore<-NA
RespData$Tscore<-NA

# Calculate Z score
RespData$Zscore <- (RespData$answer_num - OverallMean) / OverallSD

# Calculate T score
RespData$Tscore <- 10 * RespData$Zscore + 50


#calculate the average response score per department across the dataset
MeanDeptScore <- aggregate(RespData$Tscore, by = list(RespData$cluster_tag), mean)
...
MeanDeptScorePerSchool <- aggregate(RespData$Tscore, by = list(RespData$acara_id, RespData$school_name, RespData$cluster_tag, RespData$MeanDeptScore ), mean)
...
## i = 40309
  ...
  #write plot to file
  ggsave(paste(i,"DeptCompTscore.png", sep="_"), plot=SingleSchoolPlot, path=myProps$imageDir, device="png")
}
